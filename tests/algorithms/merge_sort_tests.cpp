...
TEST_CASE("merge_sort sorts an unsorted vector") {
    ...
    dsa::algorithms::merge_sort(values, 0, values.size());

    REQUIRE(values == std::vector<int>{1, 2, 3, 4, 5, 6});
}

TEST_CASE("merge_sort keeps an already sorted vector unchanged") {
...
TEST_CASE("merge_sort sorts a reverse-sorted vector") {
...
TEST_CASE("merge_sort works with duplicate values") {
...
TEST_CASE("merge_sort works with negative values") {
...
TEST_CASE("merge_sort works with an empty vector") {
...
TEST_CASE("merge_sort works with a single-element vector") {
...
TEST_CASE("merge_sort produces sorted output for a larger mixed vector") {
